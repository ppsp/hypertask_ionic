# ************************************************************
# *** This file will not work without secrets-staging.json ***
# ************************************************************
...
Write-Output "GET VARIABLES FROM secrets-staging.json"
$json = (Get-Content ".\src\scripts\azure_create_resources\secrets-staging.json" -Raw) | ConvertFrom-Json
...
$appService = az webapp create --name hypertask$nameSuffix `
...
az webapp config appsettings set --name hypertask$nameSuffix `
--resource-group hypertask-group-$nameSuffix `
