...
Set-Variable -Name "nameSuffix" -Value "stag10"
...
Copy-Item ".\src\environments\environment.source.staging.ts" -Destination ".\src\environments\environment.prod.ts"
...
Write-Output "GIT CHECKOUT STAGING BRANCH"
...
$a = Get-Content 'F:\GIT\HyperTaskWebApi\HyperTaskWebApi\appsettings.staging.json' -raw | ConvertFrom-Json
$a.KeyVaultName = "hypertask-vault-$nameSuffix"
$a | ConvertTo-Json -depth 32| set-content 'F:\GIT\HyperTaskWebApi\HyperTaskWebApi\appsettings.staging.json'
...
jarsigner -verbose -sigalg SHA1withRSA -digestalg SHA1 -keystore F://my-release-key.keystore F://app-release-unsigned.apk hypertask -storepass "s]t[fe3=nxLg"
...
git add .
git commit -m "- Automated staging build"
git push
