...
$a = Get-Content 'F:\GIT\HyperTaskWebApi\HyperTaskWebApi\appsettings.production.json' -raw | ConvertFrom-Json
$a.KeyVaultName = "hypertask-vault-$nameSuffix"
$a | ConvertTo-Json -depth 32| set-content 'F:\GIT\HyperTaskWebApi\HyperTaskWebApi\appsettings.production.json'
...
git add .
git commit -m "- Automated prod build"
git push
